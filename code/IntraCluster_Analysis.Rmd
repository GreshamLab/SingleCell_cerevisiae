...
For each bin, calculate coefficient of variation and build a linear model of coefficient of variation vs mean. Then plot the model line, calculate the residuals, and adjsut them based on the mean. All the intermediate results are saved in a list and accessible for performing further analysis.  
...
for (i in 1:length(levels(df$time_interval))) {
  ...
  tmp <- df %>% #swap to df once done developing
  ...
  #normalize df and calculate stats
  tmp <- tmp*10000/rowSums(tmp) #normalize
  tmp_mean = apply(tmp, 2, mean, na.rm = T) #average exp of each gene
  tmp_sd = apply(tmp, 2, sd) #average sd of each gene
  tmp_CV = tmp_sd/tmp_mean #calculate coefficient of variation
  tmp_stats <- rbind(mean = tmp_mean, #merge stats in one df
                  sd = tmp_sd, 
                  CV = tmp_CV)

  #transpose and add info
  tmp_stats <- as.data.frame(t(tmp_stats)) #transpose to make df ggplot friendly
  tmp_stats <- rownames_to_column(tmp_stats, "genes") #add gene name column
  tmp_stats <- filter(tmp_stats, mean > 0.01) %>% drop_na() #filter low expr
  row.names(tmp_stats) <- tmp_stats$genes #add gene name col
  ...
  tmp_residuals <- data.frame(genes = names(tmp_model$residuals), 
                              residuals = tmp_model$residuals)
                
  tmp_stats <- merge(tmp_stats, tmp_residuals, by = "genes")
  tmp_stats$adj_res <- tmp_stats$residuals/tmp_stats$mean 
  
  #add gene common name and info to df
