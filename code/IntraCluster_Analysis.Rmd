...
tmp <- rsq_matrix %>%
  ...
  filter(value < 100) %>% #remove the genes matching with themselves
  arrange(desc(value))
...
Do clustering on the matrix

```{r}
tmp <- as.dist(1 - rsq_matrix)
...
The two genes are facing each other on the genome.
