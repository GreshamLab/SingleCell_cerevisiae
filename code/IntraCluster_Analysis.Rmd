...
stats_list %>%
  ...
  select(c("genes", "common_name", "mean", "adj_res", "Time_Point")) %>%
  ...
  facet_wrap(common_name ~ variable, 
             scales = "free",
             ncol = 2) #facet grid to have mean and CV
rm("cell_cycle_genes")
...
rank_combine <- 
  ...
  map(~.x %>%
        ...
        drop_na() %>%
        mutate(rank = rank(adj_res)) %>% #rank based on adjusted residuals
