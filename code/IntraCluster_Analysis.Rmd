...
## Check if they fit negative binomial distribution
...
```{r}
df %>%
  select(c("YML028W", "time_interval")) %>% #select the interesting genes
  ggplot(aes(x = time_interval,
             y = YML028W)) +
  geom_violin()
...
tmp <- stats_list %>%
  map(~ filter(.x, mean < 4)) %>% #keep genes with low counts
  map(~ filter(.x, #keep genes with high rank
