...
```{r, fig.height=60, fig.width=10}
df %>%
  select(c(all_of(common_genes))) %>% #select the least variable genes
  cbind(df[5848:5850]) %>% #keep metadata as well
  melt(id = c("Cell_Cycle_Phase", "Cell_Cycle_Time", "time_interval"),
       value.name = "counts",
       variable.name = "genes") %>% #melt for ggplot
...
```{r}
df %>%
  select(c("YOR382W", "YOR383C")) %>% #select the least variable genes
  cbind(df[5848:5850]) %>% #keep metadata as well
...
tmp <- stats_list %>%
  map(~ filter(.x, mean < 3)) %>% #keep genes with low counts
  map(~ filter(.x, #keep genes with high rank
               genes %in% unique(grep(paste(rank_combine$genes[1:200],
                                            collapse="|"),
                                            genes,
                                            value = T))))
...
rsq_matrix <- rsq_matrix %>%
  ...
  mutate(value = value*100,
         value = round(value, 2)) %>% #express R sequared as %
  arrange(desc(value))
