#Loading the data with the ReadSOLOfunction in Seurat kept giving me error, 
#so I took the Seurat function and fixed the issue in here. 
...
readSTARsoloCounts <- 
           ...
           strip.suffix = FALSE) 
  {
    ...
    for (i in seq_along(along.with = all.files)) {
      ...
      err <- paste("Cannot find", names(x = all.files)[i], 
                   "at", uri)
    ...
    feature.names <- read.table(file = all.files[["feature list"]],
                                quote = "", #absence of this line gave error, everything else works
                                header = FALSE, 
                                sep = feature.sep, 
                                row.names = NULL, 
                                skip = skip.feature)
    ...
    if (bcols < cell.column) {
      stop("cell.column was set to ", cell.column, " but ", 
           cells, " only has ", bcols, " columns.", " Try setting the cell.column argument to a value <= to ", 
           bcols, ".")
    }
    ...
    if (fcols < feature.column) {
      stop("feature.column was set to ", feature.column, " but ", 
           features, " only has ", fcols, " column(s).", " Try setting the feature.column argument to a value <= to ", 
           fcols, ".")
    }
    if (any(is.na(x = feature.names[, feature.column]))) {
      ...
      if (replacement.column > fcols) {
        stop("Some features names are NA in column ", feature.column, 
             ". Try specifiying a different column.", call. = FALSE)
      }
      else {
    ...
    if (length(x = feature.names) != nrow(x = data)) {
      stop("Matrix has ", nrow(data), " rows but found ", length(feature.names), 
           " features. ", ifelse(test = length(x = feature.names) > 
